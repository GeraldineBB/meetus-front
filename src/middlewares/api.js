...
import { LOAD_CATEGORIES_FOR_HOME, LOAD_EVENTS_FOR_HOME, setEventForHome, setCategoriesForHome } from '../actions/events';
  ...
  switch (action.type) {
    case LOAD_EVENTS_FOR_HOME: {
      ...
      api.get ('/events?limit=3', {
        ...
        (error) => console.log('on a une erreur sur les 3 events de la home', error),
      );
    ...
    case LOAD_CATEGORIES_FOR_HOME: {
      ...
      api.get('/categories?limit=6', {
        ...
        (error) => console.log('on a une erreur sur les 6 categories de la home', error), 
      ); 
      next(action);
      break;
